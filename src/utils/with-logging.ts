...
export const withLogging =
  ...
  async (ctx: any) => {
    ...
    const rawXff = headers ? (headers.get?.("x-forwarded-for") || headers.get?.("x-forwarded") || headers.get?.("forwarded-for")) : null;
    // prefer the first IP in X-Forwarded-For (original client)
    let clientIP: string | null = null;
    if (rawXff) {
      clientIP = String(rawXff).split(",")[0]?.trim() ?? null;
    } else if (headers && (headers.get?.("x-real-ip") || headers.get?.("x-client-ip") || headers.get?.("cf-connecting-ip"))) {
      clientIP = headers.get("x-real-ip") || headers.get("x-client-ip") || headers.get("cf-connecting-ip") || null;
    ...
    try {
      ...
      else if (typeof rawStatus === "string" && !Number.isNaN(Number(rawStatus))) status = Number(rawStatus);
